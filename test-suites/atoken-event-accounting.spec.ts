...
//
//     // approve protocol to access depositor wallet
//     await waitForTx(await dai.connect(borrower.signer).approve(pool.address, MAX_UINT_AMOUNT, {gasLimit: 10000000000}));
//
...
//       .connect(borrower.signer)
//       .repay(dai.address, MAX_UINT_AMOUNT, RateMode.Variable, borrower.address, {gasLimit: 10000000000})
//
...
//       .sub(secondDaiBorrow)
//       .sub(accruedDebt2);
//     const borrowerDaiData = await helpersContract.getUserReserveData(dai.address, borrower.address, {gasLimit: 10000000000});
